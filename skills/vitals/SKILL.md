...
```
Bash(command="KNOWLEDGE_SCRIPTS=$(ls -d ~/.claude/plugins/cache/*/knowledge/*/node_modules/@agiterra/knowledge-tools/scripts 2>/dev/null | sort -V | tail -1) && python3 $KNOWLEDGE_SCRIPTS/journal.py recent 20 2>/dev/null | grep -c \"$(date +%Y-%m-%d)\"")
```
...
```
Bash(command="KNOWLEDGE_SCRIPTS=$(ls -d ~/.claude/plugins/cache/*/knowledge/*/node_modules/@agiterra/knowledge-tools/scripts 2>/dev/null | sort -V | tail -1) && python3 $KNOWLEDGE_SCRIPTS/journal.py search 'compression test score' 2>/dev/null | head -5")
```
